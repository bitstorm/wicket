#!/bin/bash
echo -n "Reverting release 7.15.0
...
git branch -D build/wicket-7.15.0
git tag -d rel/wicket-7.15.0

# clean up staging repository
git push staging --delete refs/heads/build/wicket-7.15.0
git push staging --delete rel/wicket-7.15.0

# clean up staging dist area
svn rm https://dist.apache.org/repos/dist/dev/wicket/7.15.0 -m "Release vote has failed"
