#!/bin/bash
echo -n "Reverting release 10.8.0
...
git checkout master
git branch -D build/wicket-10.8.0
git tag -d rel/wicket-10.8.0

# clean up staging repository
git push staging --delete refs/heads/build/wicket-10.8.0
git push staging --delete rel/wicket-10.8.0

# clean up staging dist area
svn rm https://dist.apache.org/repos/dist/dev/wicket/10.8.0 -m "Release vote has failed"
