...
git push origin rel/wicket-8.6.0

# promote the source distribution by moving it from the staging area to the release area

svn mv https://dist.apache.org/repos/dist/dev/wicket/8.6.0 https://dist.apache.org/repos/dist/release/wicket -m "Upload release to the mirrors"
...
# Renumber the next development iteration 8.7.0-SNAPSHOT:

git checkout wicket-8.x
mvn release:update-versions --batch-mode
mvn versions:set versions:commit -DnewVersion=8.7.0-SNAPSHOT
git add --all
...
Remove the previous version of Wicket using this command:
  ...
  svn rm https://dist.apache.org/repos/dist/release/wicket/8.5.0 -m \"Remove previous version from mirrors\"
