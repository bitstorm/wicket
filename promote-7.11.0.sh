...
git push origin rel/wicket-7.11.0

# promote the source distribution by moving it from the staging area to the release area

svn mv https://dist.apache.org/repos/dist/dev/wicket/7.11.0 https://dist.apache.org/repos/dist/release/wicket -m "Upload release to the mirrors"
...
# Renumber the next development iteration 7.12.0-SNAPSHOT:

git checkout wicket-7.x
mvn release:update-versions --batch-mode
mvn versions:set versions:commit -DnewVersion=7.12.0-SNAPSHOT
git add --all
...
Remove the previous version of Wicket using this command:
  ...
  svn rm https://dist.apache.org/repos/dist/release/wicket/7.10.0 -m \"Remove previous version from mirrors\"
