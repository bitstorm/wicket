#!/bin/bash
echo "Promoting release 9.0.0-M3
...
git push origin rel/wicket-9.0.0-M3

# promote the source distribution by moving it from the staging area to the release area

svn mv https://dist.apache.org/repos/dist/dev/wicket/9.0.0-M3 https://dist.apache.org/repos/dist/release/wicket -m "Upload release to the mirrors"
...
# Renumber the next development iteration 9.0.0-M4-SNAPSHOT:

git checkout master
mvn release:update-versions --batch-mode
mvn versions:set versions:commit -DnewVersion=9.0.0-M4-SNAPSHOT
git add --all
...
Remove the previous version of Wicket using this command:
  ...
  svn rm https://dist.apache.org/repos/dist/release/wicket/9.0.0-M3-SNAPSHOT -m \"Remove previous version from mirrors\"
