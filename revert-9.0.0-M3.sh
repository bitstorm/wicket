#!/bin/bash
echo -n "Reverting release 9.0.0-M3
...
git checkout master
git branch -D build/wicket-9.0.0-M3
git tag -d rel/wicket-9.0.0-M3

# clean up staging repository
git push staging --delete refs/heads/build/wicket-9.0.0-M3
git push staging --delete rel/wicket-9.0.0-M3

# clean up staging dist area
svn rm https://dist.apache.org/repos/dist/dev/wicket/9.0.0-M3 -m "Release vote has failed"
