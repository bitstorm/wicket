#!/bin/bash
echo -n "Reverting release 8.19.0
...
git branch -D build/wicket-8.19.0
git tag -d rel/wicket-8.19.0

# clean up staging repository
git push staging --delete refs/heads/build/wicket-8.19.0
git push staging --delete rel/wicket-8.19.0

# clean up staging dist area
svn rm https://dist.apache.org/repos/dist/dev/wicket/8.19.0 -m "Release vote has failed"
